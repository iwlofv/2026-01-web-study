...
- 학생별 작업물은 반드시 본인 GitHub 아이디 브랜치의 `projects/<github-id>/` 아래에 정리합니다.
- 작업은 `main`에서 직접 하지 않고, 본인 GitHub 아이디 이름의 브랜치에서 진행합니다.
- `main` 브랜치에는 공용 자료만 두고, 학생 제출물은 두지 않습니다.
...
## 과제 제출 방법
1. 이 레포를 `clone` 합니다.
2. 최신 `main`을 받은 뒤, 본인 GitHub 아이디 이름으로 브랜치를 만듭니다.
3. 본인 브랜치 안에서 본인 폴더를 `projects/<github-id>/` 형태로 만들거나 기존 폴더를 사용합니다.
4. 주차별 과제는 `projects/<github-id>/week1`, `projects/<github-id>/week2`처럼 정리합니다.
5. 작업 후 변경 사항을 커밋하고 본인 브랜치로 `push`합니다.
6. GitHub에서 `Pull Request`를 열어 제출합니다.
...
그 다음 GitHub에서 `pahaha404 -> main` Pull Request를 생성합니다.
