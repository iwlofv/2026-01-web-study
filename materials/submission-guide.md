...
- 다만 작업은 `main`에서 직접 하지 않고, 본인 GitHub 아이디 이름의 브랜치에서 진행합니다.
- 본인 작업물은 본인 브랜치의 `projects/<github-id>/` 아래에 정리합니다.
- `main` 브랜치에는 학생 제출물을 두지 않습니다.
...
## 브랜치 이름 권장 규칙
- 브랜치 이름은 본인 GitHub 아이디 하나로 사용합니다.
- 예: `pahaha404`

## 제출 순서
1. 이 레포를 `clone` 한 뒤 최신 `main`을 가져옵니다.
2. 본인 GitHub 아이디 이름으로 브랜치를 만듭니다.
3. 본인 폴더가 없다면 `projects/<github-id>/`를 만들고, 과제 파일을 `projects/<github-id>/week1/` 같은 위치에 정리합니다.
4. 변경 사항을 확인한 뒤 커밋합니다.
5. 본인 브랜치로 `push`합니다.
6. GitHub에서 `main` 대상으로 Pull Request를 생성합니다.
7. 운영자는 제출 확인 후 PR을 리뷰하고 닫습니다. 학생 제출 PR은 기본적으로 merge하지 않습니다.
